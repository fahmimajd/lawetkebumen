...
class OutgoingMessagePersister
{
     ...
     * @return array{message: Message, conversation: Conversation, updated: bool}|null
     */
    public function persist(array $data): ?array
    {
        ...
        $displayName = $isGroup ? $groupSubject : ($pushName ?: ($contactPhone !== '' ? $contactPhone : null));
        ...
        if (! $isGroup && ! $pushName && ($contact->display_name === null || $contact->display_name === '') && $contactPhone !== '') {
            $updates['display_name'] = $contactPhone;
        }
        if ($updates) {
        ...
        $pending = $this->findPendingMessage($conversation->id, $type, $body, $waTimestamp, $mediaMeta);
        if ($pending) {
            if ($waMessageId) {
                $pending->wa_message_id = $waMessageId;
            }
            $pending->wa_timestamp = $waTimestamp;
            $pending->status = 'sent';
            $pending->error_code = null;
            $pending->error_message = null;
            if ($replyToMessage && ! $pending->reply_to_message_id) {
                $pending->reply_to_message_id = $replyToMessage->id;
            }
            $pending->save();
        ...
        $senderWaId = is_string($data['sender_wa_id'] ?? null) ? (string) $data['sender_wa_id'] : '';
        $senderName = $data['sender_name'] ?? null;
        $senderPhone = is_string($data['sender_phone'] ?? null) ? (string) $data['sender_phone'] : '';
        ...
        $message = $waMessageId
            ? Message::where('wa_message_id', $waMessageId)->first()
            : Message::where('client_message_id', $clientMessageId)->first();

        if (! $message) {
            return null;
        }

        $previewSource = $body ?: ('['.$type.']');
    ...
    private function findPendingMessage(
        int $conversationId,
        string $type,
        ?string $body,
        Carbon $waTimestamp,
        ...
        $query = Message::query()
            ...
            ->whereBetween('created_at', [
                $waTimestamp->copy()->subMinutes(5),
                $waTimestamp->copy()->addMinutes(5),
            ]);
        ...
        if (! empty($mediaMeta['mime'])) {
            $query->where('media_mime', (string) $mediaMeta['mime']);
        }

        if (! empty($mediaMeta['size'])) {
            $query->where('media_size', (int) $mediaMeta['size']);
        }
